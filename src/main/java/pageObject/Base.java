...
public class Base {
    ...
    protected final int driverWaitTime =3;

    //Прикручиваем конструктор
    ...
    public Boolean waitVisibilityElement(final WebElement element){
        ...
        //объект, который получает на вход наш драйвер и ждёт driverWaitTime секунд
        WebDriverWait wait = new WebDriverWait(driver,driverWaitTime);

        try {
            //ждём, пока не появится элемент
        ...
        return false;
        //return waitVisibilityElement(element, driverWaitTime);
    }
